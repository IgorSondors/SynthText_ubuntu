...
def dot_word_crop(tg_alpha, b, img_counter, img_gray, word, w_of_next_ch_word, h_of_next_ch_word, recognizer_label, word_counter,   
    ...
    check_coords = True # just for examine of generation
    if check_coords:
    ...
    if check_coords:
        ...
        for i in coordinates_old: # Very slow
            # Horizontal border
            img_rgb = cv2.circle(img_gray, (i[0], i[1]), radius=0, color=(255, 255, 255), thickness=2)
    ...
    for k in dsets:
        ...
        for j in range(len(new_txt)): # word quantity
            ...
            while i < num_ch_per_w:
                i = i + 1
                #print('Character = ', all_symbols[ochered])
            ...
            try:      
                ...
                x_down_right_word, y_down_left_word = dot_word_crop(tg_alpha, b, img_counter, img_gray, word, w_of_next_ch_word, h_of_next_ch_word, 
                                                                    recognizer_label, word_counter,   
                                                                    x_down_left_word, x_down_right_word, x_top_left_word, x_top_right_word, 
                                                                    y_top_left_word, y_top_right_word)
                recognizer_label.write('\n')
            except:
                print('Bad word! This should remove manually', '\n', 'image_{}_{}.jpg'.format(img_counter, word_counter))                      
    db.close()
